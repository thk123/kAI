    ...
    public class TextBlockRenderer : IDisposable
    {
        ...
        private void CreateCharTable(byte BytePrefix)
        {
            ...
            for (int i = 0; i < 256; ++i)
            {
                ...
                CD.OverhangLeft = TL[i].OverhangMetrics.Left + 1;
                CD.OverhangTop = TL[i].OverhangMetrics.Top + 1;
                //Make XPos + CD.Overhang.Left an integer number in order to draw at integer positions
                CD.OverhangLeft += (float)Math.Ceiling(XPos + CD.OverhangLeft) - (XPos + CD.OverhangLeft);
                //Make YPos + CD.Overhang.Top an integer number in order to draw at integer positions
                CD.OverhangTop += (float)Math.Ceiling(YPos + CD.OverhangTop) - (YPos + CD.OverhangTop);

                CD.OverhangRight = CharWidth - CD.CharSize.X - CD.OverhangLeft;
                CD.OverhangBottom = CharHeight - CD.CharSize.Y - CD.OverhangTop;

                CD.TexCoordsStart = new Vector2(((float)XPos / SizeX), ((float)YPos / SizeY));
                CD.TexCoordsSize = new Vector2((float)CharWidth / SizeX, (float)CharHeight / SizeY);
